...
export function DataTable<T extends Record<string, unknown>>({
  ...
  return (
    <div className="rounded-2xl border border-[rgb(var(--border))]">
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm">
          <thead className="bg-[rgb(var(--muted))]/10 text-right">
            <tr>
          ...
          <tbody>
            {pageRows.length ? (
              pageRows.map((row, rowIndex) => (
                <tr
                  key={rowIndex}
                  className="border-t border-[rgb(var(--border))] hover:bg-[rgb(var(--muted))]/5"
                >
                  {columns.map((column, columnIndex) => {
                    const rawValue = column.cell
                      ? column.cell({ row })
                      : readValue(row, column.accessorKey);
      ...
      <div className="flex items-center justify-end gap-2 p-2 text-sm">
        ...
        <button
          type="button"
          className="rounded border border-[rgb(var(--border))] px-2 py-1 disabled:opacity-50"
